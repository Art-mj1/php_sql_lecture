...
<body>
 ...
 <?php
     ...
     //post: 보내는 데이터 양이 많고, 특히 데이터베이스에 '입력해야 할 데이터'를 보낼 때 사용한다.
     //참조: https://noahlogs.tistory.com/35

     //get:$_GET['name data'];
     //post:$_POST['name data'];
     //e둘 다 받을 수 있는 형식:$_REQUEST['name data'];
     $member_id = $_POST['id'];
     $name = $_POST['name'];
     ...
     // 1. 데이터조회: SELECT [FIELD] (*) FROM [TABLE_NAME]
     ...
     //5. 데이터 값 변경:UPDATE [TABLE_NAME] SET [변경할 필드 명]= [변경 값] WHERE [변경할 필드 조건]
     ...
     $result = mysqli_query($db_conn,$sql);
      // mysqli_query(); pPHP에서 데이터베이스에 특정 작업을 요청하는 함수
      //첫 번째 파라미터로 접속 정보를 , 두 번째 파라미터로 요청 작업을 전달한다.

  //조회 데이터를 배열 형태로 추출: mysqli_fecth array()
